...
*/
component serializable="false" implements="wirebox.system.cache.ICacheProvider"{
    ...
    */
	CFProvider function init() output=false{
		...
		instance = {
			...
			elementCleaner		= CreateObject("component","wirebox.system.cache.util.ElementCleaner").init(this),
			// Utilities
	...
	*/
	...
	/*
	* Get the cache statistics object as wirebox.system.cache.util.ICacheStats
	* @colddoc:generic wirebox.system.cache.util.ICacheStats
	*/
	any function getStats() output=false{
		return CreateObject("component", "wirebox.system.cache.providers.cf-lib.CFStats").init( getObjectStore().getStatistics() );		
	}
