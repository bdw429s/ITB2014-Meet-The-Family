...
<cfcomponent output="false" hint="This is a CacheBox configuration object.  You can use it to configure a CacheBox instance">
	...
	<cfscript>
		...
		DEFAULTS = {
			logBoxConfig = "wirebox.system.cache.config.LogBox",
			cacheBoxProvider = "wirebox.system.cache.providers.CacheBoxProvider",
			coldboxAppProvider = "wirebox.system.cache.providers.CacheBoxColdBoxProvider",
			scopeRegistration = {
				enabled = true,
				scope = "application",
				key = "cachebox"
	...
	<cffunction name="cache" output="false" access="public" returntype="any" hint="Add a new cache configuration.">
		...
		<cfargument name="provider" 	type="string" required="false"  default="#variables.DEFAULTS.cacheBoxProvider#" hint="The cache provider class, defaults to: wirebox.system.cache.providers.CacheBoxProvider"/>
