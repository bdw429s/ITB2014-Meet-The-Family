...
<!----------------------------------------- CONSTRUCTOR ------------------------------------->
	...
	<cffunction name="init" access="public" returntype="CacheFactory" hint="Constructor" output="false" >
		<cfargument name="config"  		required="false" hint="The CacheBoxConfig object or path to use to configure this instance of CacheBox. If not passed then CacheBox will instantiate the default configuration." colddoc:generic="wirebox.system.cache.config.CacheBoxConfig"/>
		<cfargument name="coldbox" 		required="false" hint="A coldbox application that this instance of CacheBox can be linked to, if not using it, just ignore it." colddoc:generic="wirebox.system.web.Controller"/>
		...
		<cfscript>
			...
			if( NOT structKeyExists( arguments, "config" ) ){
				...
				arguments.config = createObject( "component", "wirebox.system.cache.config.CacheBoxConfig" ).init( CFCConfigPath=defaultConfigPath );
			}
			else if( isSimpleValue( arguments.config ) ){
				arguments.config = createObject( "component", "wirebox.system.cache.config.CacheBoxConfig" ).init( CFCConfigPath=arguments.config );
			}
	...
	<cffunction name="configure" output="false" access="public" returntype="void" hint="Configure the cache factory for operation, called by the init(). You can also re-configure CacheBox programmatically.">
		<cfargument name="config" required="true" hint="The CacheBoxConfig object to use to configure this instance of CacheBox" colddoc:generic="wirebox.system.cache.config.CacheBoxConfig"/>
		<cfscript>
...
<!------------------------------------------- PUBLIC CACHE FACTORY OPERATIONS ------------------------------------------>
	...
	<!--- getCache --->
    <cffunction name="getCache" output="false" access="public" returntype="any" hint="Get a reference to a registered cache in this factory.  If the cache does not exist it will return an exception. Type: wirebox.system.cache.ICacheProvider" colddoc:generic="wirebox.system.cache.ICacheProvider">
    ...
    <cffunction name="addCache" output="false" access="public" returntype="void" hint="Register a new instantiated cache with this cache factory">
    	<cfargument name="cache" required="true" hint="The cache instance to register with this factory of type: wirebox.system.cache.ICacheProvider" colddoc:generic="wirebox.system.cache.ICacheProvider"/>
    	<cfset registerCache( arguments.cache )>
	</cffunction>

	<!--- addDefaultCache --->
    <cffunction name="addDefaultCache" output="false" access="public" returntype="any" hint="Add a default named cache to our registry, create it, config it, register it and return it of type: wirebox.system.cache.ICacheProvider" colddoc:generic="wirebox.system.cache.ICacheProvider">
    ...
    <cffunction name="removeFromScope" output="false" access="public" returntype="void" hint="Remove the cache factory from scope registration if enabled, else does nothing">
    	<cfscript>
			...
			if( scopeInfo.enabled ){
				createObject("component","wirebox.system.core.collections.ScopeStorage").init().delete(scopeInfo.key, scopeInfo.scope);
			}
		</cfscript>
    </cffunction>
    ...
    <cffunction name="replaceCache" output="false" access="public" returntype="void" hint="Replace a registered named cache with a new decorated cache of the same name.">
    	<cfargument name="cache" 			required="true" hint="The name of the cache to replace or the actual instance of the cache to replace" colddoc:generic="wirebox.system.cache.ICacheProvider"/>
		<cfargument name="decoratedCache" 	required="true" hint="The decorated cache manager instance to replace with of type wirebox.system.cache.ICacheProvider" colddoc:generic="wirebox.system.cache.ICacheProvider"/>
...
<!----------------------------------------- PUBLIC PROPERTY RETRIEVERS ------------------------------------->
	...
	<!--- getColdbox --->
    <cffunction name="getColdbox" output="false" access="public" returntype="any" hint="Get the instance of ColdBox linked in this cache factory. Empty if using standalone version" colddoc:generic="wirebox.system.web.Controller">
	...
	<!--- getLogBox --->
    <cffunction name="getLogBox" output="false" access="public" returntype="any" hint="Get the instance of LogBox configured for this cache factory" colddoc:generic="wirebox.system.logging.LogBox">
	...
	<cffunction name="getConfig" access="public" returntype="any" output="false" hint="Get this LogBox's configuration object." colddoc:generic="wirebox.system.cache.config.CacheBoxConfig">
	...
	<!--- getDefaultCache --->
    <cffunction name="getDefaultCache" output="false" access="public" returntype="any" hint="Get the default cache provider of type wirebox.system.cache.ICacheProvider" colddoc:generic="wirebox.system.cache.ICacheProvider">
    ...
    <!--- createCache --->
    <cffunction name="createCache" output="false" access="public" returntype="any" hint="Create a new cache according the the arguments, register it and return it of type: wirebox.system.cache.ICacheProvider" colddoc:generic="wirebox.system.cache.ICacheProvider">
...
<!----------------------------------------- PRIVATE ------------------------------------->
    ...
    <cffunction name="doScopeRegistration" output="false" access="private" returntype="void" hint="Register this cachefactory on a user specified scope">
    	<cfscript>
    		var scopeInfo 		= instance.config.getScopeRegistration();
			var scopeStorage	= createObject("component","wirebox.system.core.collections.ScopeStorage").init();
    ...
    <cffunction name="configureLogBox" output="false" access="private" returntype="void" hint="Configure a standalone version of logBox for logging">
    	<cfscript>
			...
			var config = createObject("component","wirebox.system.logging.config.LogBoxConfig").init(CFCConfigPath="wirebox.system.cache.config.LogBox");
			// Create LogBox
