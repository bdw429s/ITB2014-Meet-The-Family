component displayname="User Service" hint="I look after the User Business Logic" output="false"
{
	...
	public numeric function checkLogin(email, password) {
		...
		if ( len(arguments.email) lte 5) {
			// email too short
			log.warn("#cgi.remote_addr# entered an email that was too short");
			return 0;
		}
		else if ( len(arguments.password) eq 0 ) {
			// empty password
			log.warn("#cgi.remote_addr# entered an empty password");
			return 0;
		}
		else {
			var userDAO = uDAO;
			...
			if ( user.recordcount eq 0) {
				log.warn("#cgi.remote_addr# entered invalid credentials");
